...
**项目放哪都行**（推荐 `文档\<插件名>\`）：插件 SDK 从 NuGet 装（`luolan.winland.Core`，版本号 = 宿主 API 版本，当前 2.2.0），不依赖 WinIsland 源码。唯一**必须改**的是 csproj 里的 `WinIslandPluginsDir` → 指向你机器上宿主的 `plugins` 目录（安装版默认 `%LocalAppData%\Programs\WinIsland\plugins`），这样 `dotnet build` 会直接把插件装进你平时用的 WinIsland。
...
- `MyPlugin.csproj` —— 工程文件：从 NuGet 引用插件 SDK（`luolan.winland.Core 2.2.0`）、排除宿主自带的运行时、构建后自动拷进 `WinIslandPluginsDir`
