...
| 「插件针对 WinIsland.Core x.y 构建，与宿主不兼容」 | 用了 1.x 的 NuGet 包，或旧版编译的 dll | 用 2.x 的 SDK 重新编译：csproj 里 `<PackageReference Include="luolan.winland.Core" Version="2.2.1" …>`（`api_version` 2） |
