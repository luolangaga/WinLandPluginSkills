...
类型由宿主**按内容判定，是一条退化链**（真文件 → 图片 → 文本）：「真文件」要求有真实路径；浏览器拖网页图片给的是**虚拟文件**（没有路径，但宿主会读内容，
按扩展名认出是图片就转成图片载荷，SVG 也支持）；同时有位图和文本时不会盲目当图片 —— 浏览器里拖选中的文字常附带一张「选区快照」位图，那种情况按文本处理，
只有 HTML 片段里带 `<img>` 或文本本身就是图片地址才算图片；每一步读取都有超时，内容读不出来时面板会直接收回（不会停在"正在读取…"）。
判定依据与可用格式会写进宿主日志的 Debug 行，排错先看它。
